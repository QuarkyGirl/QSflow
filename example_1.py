...
cs = ['#000000','#FF1F1F','#2274A5']

# plot potentials
qsea, = ax.plot(φ,U[-1],ls='-',c=cs[0],zorder=2,lw=0.7)
one_loop, = ax.plot(φ,v_eff,c=cs[1],ls='--',zorder=4,lw=0.7,dashes=[4,4])
tree, = ax.plot(φ,v_tree,ls='-',c=cs[2],zorder=1,lw=0.7)

# add legends
leg1 = plt.legend(
(qsea,one_loop,tree),
(r'QSEA+FRG: $U_{k = 0}(\phi)$',r'One-loop: $V_\mathrm{eff}(\phi)$',r'Tree-level: $V(\phi)$'),
...
# save and show
plt.savefig('figs/example_1.pdf',bbox_inches = 'tight',pad_inches=0.01)
plt.show()
